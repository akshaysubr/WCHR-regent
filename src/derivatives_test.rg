...
task main()
  ...
  var token = initialize(coords, r_prim_c, r_prim_l_x, r_prim_l_y, r_prim_l_z, dx, dy, dz, n_ghosts)
  wait_for(token)
  ...
  fill(r_der.rho, 0.0)
  c.printf("Setting matrix\n")
  c.printf("Getting LU decomposition\n")
  for i in pencil_interior do
    get_derivative_matrix(p_mat_x[i], beta06MND, alpha06MND, 1.0, alpha06MND, beta06MND)
    get_LU_decomposition(p_LU_x[i], p_mat_x[i])
  end
  c.printf("Computing MND derivative\n")
  for i in pencil_interior do
