...
problem.boundary_l_x = { condition="SUBSONIC_INFLOW",  rho=1., u=problem.velocity, v=0., w=0., p=1., L_x = 0.1, eta_2 = 2.0, eta_5 = 2.0 }
problem.boundary_r_x = { condition="SUBSONIC_OUTFLOW", p=1., L_x = 0.1, sigma = 0.005 }
...
do
  for i in coords.ispace do
    ...
    var exponent = (coords[i].x_c - problem.X1 - 0.5*problem.LX)/problem.width
    r_prim_c[i].rho = 1.0 + problem.epsilon * cmath.exp(-exponent*exponent)
    r_prim_c[i].u   = problem.velocity
    r_prim_c[i].v   = 0.0 
    r_prim_c[i].w   = 0.0
    r_prim_c[i].p   = 1.0
  end
...
do
  ...
  for i in r_prim_c do
    ...
    var exponent = (coords[i].x_c - problem.velocity*tsim - problem.X1 - 0.5*problem.LX)/problem.width
    ...
    err = cmath.fabs( r_prim_c[i].u   - problem.velocity )
